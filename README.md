...
`0`/`1` is the enforced convention by default, deliberately — other integer values are ambiguous rather than safely ignorable (e.g. a nonzero value isn't reliably "true" the same way across every database or language), so an unrecognised number returns `None` rather than a guess.
...
(`is_truthy_in_range`/`TruthyRuleSet::set_min_max` exist for the rare case where a column was never really a boolean — see the note near the end of the next section.)
...
#### `set_min_max` — for when a column was never really boolean

`0`/`1` is the convention this crate enforces by default, and that shouldn't normally need changing. The one case it's for: some columns were never really booleans at all — just status/outcome codes expressed as small integer sentinels (e.g. only `5` through `10` are meaningful values, or a wider band either side of zero matters). `.set_min_max(min, max)` widens (or narrows) the numeric range `is_truthy_in_range` accepts; the only requirement is `max > min`, an invalid range (`min >= max`) is silently ignored and the default is kept, and truthiness within the range is always `value > 0`:

```rust
use is_truthy::TruthyRuleSet;

// a status-code column where only 5 through 10 are valid, and all of them read as "true"
...
assert_eq!(rules.parse("4"), None);  // below the window
